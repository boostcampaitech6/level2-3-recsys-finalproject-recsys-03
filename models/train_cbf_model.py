...
from graphsage_cbf.args_cbf import parse_args
from graphsage_cbf.data_preprocessing import data_preprocessing
from graphsage_cbf.model import Model
from graphsage_cbf.trainer import train, test, feature
from graphsage_cbf.utils import set_seed, makedirs, get_logger
...
def main():
    ...
    # Edge 저장
    sparse_size = (train_data['tag'].num_nodes, train_data['track'].num_nodes)
    ...
    train_loader = LinkNeighborLoader(
        train_data,
        num_neighbors = {('tag', 'tagged', 'track'): [args.neighbors_sampling] * args.n_layers,    # [node 당 sample 개수] * layer 개수
                         ('track', 'rev_tagged', 'tag'): [args.neighbors_sampling] * args.n_layers},
        edge_label_index = (('tag', 'tagged', 'track'), train_data['tag', 'tagged', 'track'].edge_index),
    ...
    counter = 0
    best_content_dict = {'epoch':0, 'train_loss':0, 'train_ndcg':0, 'train_recall':0, 'valid_ndcg':0, 'valid_recall':0}
    for epoch in range(1, args.epochs+1):
        ...
        logger.info(f'Train Content NDCG@{args.topk}: {train_ndcg:.4f}  Train Content Recall@{args.topk}: {train_recall:.4f}')
        ...
        logger.info(f'Valid Content NDCG@{args.topk}: {valid_ndcg:.4f}  Valid Content Recall@{args.topk}: {valid_recall:.4f}')
        
        # Best Content 모델 저장
        if valid_ndcg > best_content_dict['valid_ndcg']:
            logger.info(f'Best Content NDCG@{args.topk} is Updated')
            best_content_dict = {'epoch': epoch, 'train_loss': train_loss, 'train_ndcg': train_ndcg, 'train_recall': train_recall, 'valid_ndcg': valid_ndcg, 'valid_recall': valid_recall}
    ...
    logger.info(f'Epoch: {best_content_dict["epoch"]:02d}  Loss: {best_content_dict["train_loss"]:.4f}')
    logger.info(f'Train Content NDCG@{args.topk}: {best_content_dict["train_ndcg"]:.4f}  Train Interaction Recall@{args.topk}: {best_content_dict["train_recall"]:.4f}')
    logger.info(f'Valid Content NDCG@{args.topk}: {best_content_dict["valid_ndcg"]:.4f}  Valid Interaction Recall@{args.topk}: {best_content_dict["valid_recall"]:.4f}')
    ...
    logger.info(f'Test  Content NDCG@{args.topk}: {test_ndcg:.4f}   Test  Content Recall@{args.topk}: {test_recall:.4f}')
