...
def parse_args():
    ...
    parser.add_argument('--tag_filename', default='tag_context.csv', type=str, help='tag 파일명 (csv 형식)')
    parser.add_argument('--model_name', default='tag_model', type=str, help='모델명')
    ...
    # 추론 파라미터
    parser.add_argument('--input_tag_list', default=['love','kpop','dance','happy','joy'], type=list, help='input tag list (list 형식)')
    parser.add_argument('--graph_filename', default='graph_data_tag_model.pt', type=str, help='그래프 데이터 파일명')
    parser.add_argument('--model_filename', default='tag_model.pt', type=str, help='모델 파일명')
