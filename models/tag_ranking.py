...
def tag_ranking_load_data(recommended_track_id):
    #Load Data
    sideinfo_data = pd.read_csv('./data/sideinfo.csv', index_col=0)
    sideinfo_data['tag_string'] = sideinfo_data['tags'].apply(lambda x: ' '.join(str_to_dict(x).keys()))   
