...
def load_data():
    ...
    return mapping_track_index, mapping_index_track
...
def inference(input_data, k=100):
    # 데이터 로드
    mapping_track_index, mapping_index_track = load_data()
    ...
    # 모델 로드
    model = torch.load('../data/model_versions/graphsage.pt')
...
if __name__ == "__main__":
    input_data = pd.read_csv('../data/Jinmin_2.csv', index_col=0)
    recom = inference()
    print(recom)