...
def inference(login_user_data, input_tags):   
    ...
    if login_user_data.empty or login_user_data is None: # track list가 없는 유저 (비회원)
        recommended_list = infernece_tag_model(k=100,input_tags)   
        ...
        recommended_list = inference_tag_model(k=100,input_tags)
        ...
        if not login_user_data_interaction.empty and not login_user_data_content.empty: # interaction 데이터가 부분적으로 있는 유저
            recommended_list_cf = inference_cf_model(k=10, recommended_list_filtered)
            recommended_list_cbf = inference_cbf_model(k=10, recommended_list_filtered)
        ...
        elif login_user_data_interaction.empty: # interaction 데이터가 없는 유저
            recommended_list_cbf = inference_cbf_model(k=20, recommended_list_filtered)
            recommended_playlist = recommended_list_cbf

        elif login_user_data_content.empty: # interaction 데이터가 있는 유저
            recommended_list_cf = inference_cf_model(k=20, recommended_list_filtered)
