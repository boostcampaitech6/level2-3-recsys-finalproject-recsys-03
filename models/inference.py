import pandas as pd
from models.inference_tag_model import inference_tag_model
from models.inference_cf_model import inference_cf_model, load_data_for_cf_model, update_user_graph_data, load_cf_model
from models.inference_cbf_model import inference_cbf_model
...
def inference(login_user_data, input_tags):
    ...
    if login_user_data is None:
        # tag_model (in memory global 변수 tag_model_dict 사용)
        recommended_list = inference_tag_model(100, input_tag_list, tag_model_dict['mapping_index_track'], tag_model_dict['mapping_tag_index'], tag_model_dict['embeddings'])
    ...
    # track list가 있는 유저 (회원)
        ...
        if len(recommended_list_filtered_interaction) >= 10 and len(recommended_list_filtered_content) >= 10 and len(login_user_data_interaction) >= 1 and len(login_user_data_content) >= 1:
            # cf model (input user에 따라 graph data와 cf model이 변하기 때문에 in memory 사용 불가)
            ...
            # cbf model (in memory global 변수 cbf_model_dict 사용)
            recommended_list_cbf = inference_cbf_model(10, login_user_data_content, recommended_list_filtered_content,
                                                       cbf_model_dict['mapping_index_track'], cbf_model_dict['mapping_track_index'], cbf_model_dict['embeddings'])
        ...
        # track_without_interaction이 주로 필터링된 경우
            ...
            # cbf model (in memory global 변수 cbf_model_dict 사용)
            recommended_list_cbf = inference_cbf_model(20, login_user_data_content, recommended_list_filtered_content,
                                                       cbf_model_dict['mapping_index_track'], cbf_model_dict['mapping_track_index'], cbf_model_dict['embeddings'])
